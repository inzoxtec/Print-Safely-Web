...
export default function UploadClient() {
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (let fileIndex = 0; fileIndex < processedFilesBlobs.length; fileIndex++) {
        ...
        const chunkPromises = fileChunks.map((chunk, chunkIndex) => {
          const chunkRef = doc(db, "documents", docId, "chunks", `${fileIndex}_${chunkIndex}`);
        ...
        chunksUploadedSoFar += fileChunks.length;
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-gradient-to-br from-blue-50/50 via-zinc-50 to-blue-100/30 dark:from-zinc-950 dark:via-zinc-900 dark:to-blue-950/20 px-4 py-12 transition-colors duration-300">
      ...
      {/* SEO Section & Schema for Secure Document Link Generation */}
      <ToolSeoSection
        title="Secure Document Sharing - Print-Only Links & Leak Protection"
        subtitle="Protect sensitive documents when printing at public print shops, client offices, or university centers. Generate self-destructing, print-only links with zero downloads."
        steps={[
          { title: "Upload & Encrypt", desc: "Files are encrypted locally on your device using AES-256 military-grade encryption." },
          { title: "Configure Security", desc: "Set print attempt limits, expiry duration, security PIN passcodes, or screen blur shields." },
          { title: "Share Print Link & QR", desc: "Send the secure URL or QR code to the printer shop. Documents destruct after printing." }
        ]}
        features={[
          { icon: "🛡️", title: "No Local File Downloads", desc: "Prevents print shops from saving your original PDF or document to their local disk." },
          { icon: "🔐", title: "AES-256 Client Encryption", desc: "Your document keys are split and encrypted on your client device before transmission." },
          { icon: "⌛", title: "Self-Destructing Links", desc: "Links automatically expire after a set time or after maximum print attempts are reached." }
        ]}
        faqs={[
          { question: "How does SafelyPrint prevent file downloads at print shops?", answer: "SafelyPrint renders document pages directly into HTML5 canvas elements within a restricted print viewer. Browser download buttons, right-click saving, and developer inspections are disabled." },
          { question: "Can I set a PIN password for my print links?", answer: "Yes! You can enable optional numeric PIN passcodes to ensure only authorized recipients can view and print your files." },
          { question: "What happens after the document is printed?", answer: "Once the document package reaches its maximum allowed print attempts, all encrypted file segments are permanently purged from the database." }
        ]}
      />

      <ToolSeoSchema
        name="Secure Document Link Generator"
        description="Share sensitive documents safely with print-only links. Prevent file downloads and PDF saving at print shops with client-side AES-256 encryption."
        url="https://printsafely.app/upload"
        steps={[
          "Upload your PDF, DOCX, or Image files into the secure generator.",
          "Set print limits, PIN protection, or blur shields.",
          "Share the generated print link or QR code with the printing shop."
        ]}
        faqs={[
          { question: "How does SafelyPrint prevent file downloads at print shops?", answer: "SafelyPrint renders document pages directly into HTML5 canvas elements within a restricted print viewer. Browser download buttons, right-click saving, and developer inspections are disabled." },
          { question: "Can I set a PIN password for my print links?", answer: "Yes! You can enable optional numeric PIN passcodes to ensure only authorized recipients can view and print your files." },
